...
const ReportsClient: React.FC = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      const [monthlyRes, comparativeRes, habitsRes] = await Promise.all([
        fetch(`${API_BASE}/reports/monthly`, { cache: 'no-store' }).then(res => res.json()).catch(() => ({ income: 0, expense: 0, savings: 0, top_category: 'N/A' })),
        fetch(`${API_BASE}/reports/comparative`, { cache: 'no-store' }).then(res => res.json()).catch(() => ({ current_expense: 0, prev_expense: 0, difference: 0 })),
        fetch(`${API_BASE}/reports/habits`, { cache: 'no-store' }).then(res => res.json()).catch(() => ({ top_days: [], repeated_expenses: {} }))
      ]);
  ...
  const handleDownloadPDF = async (reportType: string) => {
    ...
    try {
      switch (reportType) {
        case 'monthly':
          const response = await fetch(`${API_BASE}/reports/monthly-12`, { cache: 'no-store' });
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <button
                ...
                className="w-full mt-6 flex items-center justify-center space-x-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <button
                ...
                className="w-full mt-6 flex items-center justify-center space-x-2 px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <button
                ...
                className="w-full mt-6 flex items-center justify-center space-x-2 px-4 py-3 bg-pink-600 hover:bg-pink-700 text-white font-semibold rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <div className="mt-12 text-center">
          <button
            onClick={async () => {
              ...
              try {
                const monthlyResponse = await fetch(`${API_BASE}/reports/monthly-12`, { cache: 'no-store' });
                const monthlyData = await monthlyResponse.json();
                await generateMonthlyPDF(monthlyData);
                if (comparative) await generateComparativePDF(comparative);
                if (habits) await generateHabitsPDF(habits);
              } catch (error) {
